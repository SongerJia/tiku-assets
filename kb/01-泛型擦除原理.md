...
>
> **本篇知识点清单（34 个，按知识面五层标注，也是出题锚点）**：
>
> **基础使用**：13 泛型类定义与菱形 / 14 泛型接口两种实现 / 15 泛型类继承两种姿势 / 16 类型参数命名惯例 / 17 泛型方法 vs 泛型类
>
> **机制原理**：1 泛型解决什么问题 / 2 运行时同类 / 3 擦除规则 / 4 Signature 保留 / 5 调用处插强转 / 6 桥方法 / 7 泛型数组 / 8 泛型不变性 / 9 raw type / 26~29 高频坑
>
> **高级进阶**：30 varargs 堆污染 / 31 @SafeVarargs / 32 自限泛型原理 / 33 自限泛型应用
>
> **实战技能**：10 extends 只读 / 11 super 只写 / 12 PECS 记忆法 / 18 复杂签名四步读法 / 19~22 四个实战签名案例 / 23 反射三件套 / 24 TypeToken / 25 ResolvableType
>
> **设计权衡**：34 擦除 vs C# 运行时泛型
...
看到了吗——**descriptor（运行时用的签名）是 Number，但 Signature 属性（元数据）里是 T**。这就是"擦除但不全丢"的铁证：JVM 执行时不看 Signature，但**反射可以看到**。后面"运行时怎么拿到泛型信息"的块（第 23~25 块），全靠这个属性。

---

## 擦除的副作用：调用处的强转是谁插的？

上面 `Box<Number>` 的 `value` 字段读出来是 Number，但如果你写 `Box<Integer>`，读字段时编译器会在调用点插入 `(Integer) cast`：
...
**这是擦除的第二个隐藏动作：调用处插强转。** 泛型代码编译后，所有"把泛型值当具体类型用"的地方，javac 都会插入强转——类型检查是编译期做的，运行期靠这些强转兜底。这也解释了为什么泛型安全问题（类型真的不对时）抛的是 `ClassCastException`，而不是别的错误。
...
> 边界：这与"为什么不能创建泛型数组"是一个硬币的两面——数组协变靠运行时检查兜底所以安全，泛型想协变只能靠通配符 + 限制读写。**"不变性"不是设计失误，是擦除的必然推论**。

---

## raw type（裸类型）是什么？为什么有警告？
...
> 边界：泛型数组问题里那个"能编译的 `new List[10]`"就是 raw type——它绕过了检查，换来的是 unchecked 警告和运行期风险。**一句话：新代码出现 raw type = 代码味道**。
...
**为什么？** 一句直觉：`? extends T` 表示"**某种 T 的子类**"，具体是谁不知道——所以**能读**（读到的一定是 T，向上转型安全），**不能写**（写进去的没法保证是这个未知子类）。比如 `List<? extends Number>` 实际可能是 `List<Integer>` 也可能是 `List<Double>`——你往里塞 42，万一是 `List<Double>` 呢？编译器不敢冒险。

**用法场景**：方法"从集合取数据"时（集合是生产者），参数用 `? extends T`——读出来的东西一定是 T，安全。
...
**为什么？** 一句直觉：`? super T` 表示"**某种 T 的父类**"，具体是谁不知道——所以**能写**（T 一定能放进它的父类容器，向上转型安全），**不能读**（读出来不知道具体是哪个父类，只能当 Object）。比如 `List<? super Integer>` 实际可能是 `List<Number>` 也可能是 `List<Object>`——读出来没法确定具体类型。

**用法场景**：方法"往集合放数据"时（集合是消费者），参数用 `? super T`——放进去的一定是 T，安全。
...
## PECS 怎么记？生产者 extends，消费者 super
...
> 边界一：通配符也能做"无界" `List<?>`（只读，且不能 add 非 null），等价于"不知道任何信息"。
>
> 边界二："通配符捕获"（wildcard capture）——把 `List<?>` 借给泛型方法推断出具体类型。经典场景是交换元素：`swap(List<?> list, int i, int j)` 里不能直接 `list.set(i, list.get(j))`（? 不能写），解法是让泛型方法"捕获"通配符：
>
> ```java
...
**菱形（diamond）**：`new Box<>()` 里的 `<>` 是 JDK 7 引入的，编译器从左边声明的类型推断右边该填什么——左边是 `Box<String>`，右边 `<>` 自动推断为 `Box<String>`。这是"目标类型推断"（后面泛型方法块会再讲）。

> 边界：泛型类不能被 `new` 成具体类型之外的形式（`new Box<>()` 是菱形推断，`new Box()` 是 raw type——见"raw type"块）。
...
姿势②的意义：写"通用基础实现"时，你不知道将来被什么类型用——保留泛型参数，让下游（子类）来定。MyBatis 的 `BaseMapper<T>`、Spring Data 的 `Repository<T, ID>` 全是这个套路。
...
**一个高频误区**：泛型类之间没有继承关系（见"泛型不变性"块），但**泛型类本身可以继承**（`StringList` 继承 `ArrayList<String>`）——两者别混。前者是"类型参数的关系"，后者是"类的关系"。

---

## 泛型参数命名惯例 T/E/K/V/R

**类型参数命名惯例**（代码可读性，Oracle 教程规定）：
...
为什么约定大写单字母？**为了和类名/接口名区分**——看到 `T` 就知道是类型参数，看到 `String` 就知道是具体类。这不是风格问题，是"让读代码的人一眼分清变量和类型"的语义约定。
...
> 边界：这套读法有个前提——通配符方向（第 3 步）依赖 PECS（"PECS 怎么记"块）的理解。如果 PECS 还不熟，先回那块再看这里。另外，IDE（IDEA）悬停方法名会显示完整泛型签名，读代码时配合 IDE 高亮，比人肉拆快。

---

## 案例：Comparator<? super T> 为什么能传 Comparator<Animal>？

`Collections.sort(List<T> list, Comparator<? super T> c)`（天天用）
...
## 案例：Function<? super T, ? extends R> 的"入 super 出 extends"

`Stream.map(Function<? super T, ? extends R> mapper)`（Stream 里天天见）
...
**`TreeMap<K, V extends Comparable<? super V>>`**（Key 的自然排序）
...
**嵌套泛型 `Map<String, List<Map<String, Integer>>>`**
...
## 案例：Collections.max 的终极签名怎么读？

**`<T extends Object & Comparable<? super T>>`**（泛型方法 + 下界边界的组合，`Collections.max` 的签名）
...
注意 `& Object` 的写法：`Object` 被显式列为第一个上界——这是为了在擦除后保留 `Object` 的方法签名（老代码兼容的细节，了解即可）。

---

## 运行时怎么拿到泛型信息？反射三件套

擦除不全丢——**Signature 属性**里留着泛型"照片"，反射可以读。核心 API 三件套：`getGenericType()`（字段）、`getGenericReturnType()`（方法）、`getGenericSuperclass()`（父类）。
...
## TypeToken：Gson 怎么拿到 List<User>？

**模式：TypeToken（Gson 的泛型 JSON 反序列化）**
...
**核心思路**：`List<User>` 没法直接作为 `Type` 传递（局部变量不生成 Signature），但"写进类声明"就能留 Signature——`new TypeToken<List<User>>() {}` 创建了一个匿名子类，它的父类泛型是 `List<User>`，反射 `getGenericSuperclass()` 就能拆出来。**这就是"把泛型冻在类声明里，反射 Signature 取回"的经典应用。**

---

## ResolvableType：Spring 泛型 Mapper 的套路

**模式：Spring 的 ResolvableType（泛型解析工具）**
...
## 坑 1-2：instanceof 与 unchecked 强转
...
原因不用多说了——运行时根本没有 `ArrayList<String>` 这个类型可查（"运行时是同一个类"块的结论）。
...
> 边界：坑 3/4/5/7/8 都是"擦除"的直接后果——签名冲突、运行时类型缺失、不知道真实类。记住"运行时没有泛型信息"这一条，这些坑全都可以推导出来，而不是背下来。

---

## 泛型 varargs 为什么有警告？堆污染是什么？
...
**为什么会警告？** 因为 varargs 在底层就是数组，而 `List<T>...` 擦除后要创建一个 `List[]`——**泛型数组**！前面说过泛型数组是禁止的，但 varargs 是语法糖、由编译器偷偷创建，绕过了禁令——代价就是 unchecked 警告。
...
## 自限泛型：Enum<T extends Enum<T>> 在读什么？
...
> **一句话收尾**：泛型是编译期的"贴纸"：编译时管类型安全，运行时撕掉换 Object——但 Signature 属性留了张照片，反射能考古；桥方法补上擦除造成的签名断裂，PECS 管住通配符的读写方向。记不住别的，记住这三句就够了。
